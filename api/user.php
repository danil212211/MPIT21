...
class User {
	...
	public function signupUser($userPassword) {
			...
			$request = mysqli_query($this->database,"SELECT * FROM  `USER` WHERE `LOGIN`='".$this->userLogin."'");
			$array=mysqli_fetch_array($request);
			if ($array["LOGIN"]=="") {
			$this->userHash=$password_hash;
			$sql = mysqli_query($this->database,"INSERT INTO `USER` (`LOGIN`, `HASH` , `SALT`) VALUES ('".$this->userLogin."', '".$password_hash."','".$Salt."');");				
			return true;
			} else {
				return false;
			}
